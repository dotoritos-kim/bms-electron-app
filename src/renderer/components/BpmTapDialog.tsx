  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-5 w-80 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-sm font-semibold text-zinc-200 mb-3">{t('dialogs.bpmTap.title')}</h3>
        <div className="text-center mb-4">
          <div className="text-4xl font-bold text-blue-400 font-mono">{bpm > 0 ? bpm : '--'}</div>
          <div className="text-xs text-zinc-500 mt-1">
            {t('dialogs.bpmTap.tapsCount', { count: taps.length })}
            {wasReset && <span className="ml-1 text-yellow-400">{t('dialogs.bpmTap.resetIndicator')}</span>}
          </div>
        </div>
        <button
          onClick={handleTap}
        ...
        >
          {t('dialogs.bpmTap.tapButton')}
        </button>
        <div className="flex justify-between">
          <button onClick={() => setTaps([])} className="px-3 py-1.5 text-xs text-zinc-400 hover:text-zinc-200 rounded hover:bg-zinc-800">{t('dialogs.bpmTap.resetButton')}</button>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-3 py-1.5 text-xs text-zinc-400 hover:text-zinc-200 rounded hover:bg-zinc-800">{t('common:actions.cancel')}</button>
            <button
            ...
            >{t('dialogs.bpmTap.applyButton')}</button>
          </div>
        </div>
      </div>
    </div>
